...
const App = () => {
  ...
  useEffect(() => {
    const loadGameResults = async () => {
      ...
      if (emailAddress.length > 0) {
        setResults(
          (await loadGamesFromCloud(emailAddress, 'tca-cribbage')) ?? []
        );
      }
    };
  ...
  const addGameResult = async (gameResult: GameResult) => {
    const newResults = [...results, gameResult];
    ...
    <ThemeProvider theme={themeMode}>
      ...
      <main>
        <Routes>
          ...
          />
          <Route path='stats' element={<Stats gameResults={results} />} />
          <Route
            path='leaderboard'
            element={
              <Leaderboard
